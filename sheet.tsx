...
export default function CapacityGrid() {
  ...
  // 🗓️ États pour la fourchette de dates
  ...
  // 🗓️ 1. Extraction et filtrage par Date Range
  const displayTimeSlots = useMemo(() => {
    if (!timeSlots || timeSlots.length === 0) return []

    // Extrait unique YYYY-MM-DD
    const sorted = Array.from(
      new Set(timeSlots.map((d) => d.split("T")[0]))
    ).sort()
        ...
        <div className="shadow-xs flex flex-wrap items-center gap-4 rounded-xl border border-gray-200/80 bg-white p-3 dark:border-gray-800 dark:bg-gray-900/60 dark:backdrop-blur">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={selectedTeam} onValueChange={setSelectedTeam}>
              <SelectTrigger id="team-select" className="w-[170px] dark:border-gray-800 dark:bg-gray-950">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={selectedSprint} onValueChange={setSelectedSprint}>
              <SelectTrigger id="sprint-select" className="w-[160px] dark:border-gray-800 dark:bg-gray-950">
          ...
          {/* 🗓️ SÉLECTEUR DE DATES (FOURCHETTE) */}
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="h-9 rounded-md border border-gray-200 bg-transparent px-2.5 text-xs text-gray-900 shadow-xs outline-none transition focus:border-blue-500 dark:border-gray-800 dark:bg-gray-950 dark:text-gray-100"
              />
            ...
            {/* Bouton pour réinitialiser la période */}
            {(startDate || endDate) && (
              <Button
                variant="ghost"
                onClick={() => {
                ...
                className="h-8 px-2 text-xs text-gray-500 hover:text-gray-900 dark:hover:text-gray-200"
              >
                Réinitialiser
              </Button>
            )}
          ...
          <Table className="border-none">
            <TableHead>
              <TableRow>
                ...
                {/* EN-TÊTE : Affiche uniquement les jours filtrés par la plage de dates */}
            ...
            <TableBody>
              {filteredResources.map((resource) => {
                // Mapping Date -> Slot pour alignement strict
                ...
                resource.slots.forEach((slot, index) => {
                  ...
                  if (rawDate) {
                    const formattedKey = rawDate.split("T")[0]
